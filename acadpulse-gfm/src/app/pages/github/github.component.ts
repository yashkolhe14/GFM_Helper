  ...
  template: `
    <div class="space-y-6">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 class="text-3xl font-bold tracking-tight text-white">GitHub Profiles</h1>
          <p class="text-text-tertiary mt-1 text-sm">Monitor student open-source contributions and profiles.</p>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="glass-panel p-5 border-l-4 border-l-accent-primary relative overflow-hidden group hover:translate-y-[-2px] transition-all duration-300">
          ...
          <div class="absolute -right-4 -bottom-4 opacity-5 group-hover:opacity-10 transition-opacity">
            <mat-icon class="text-7xl">code</mat-icon>
          </div>
        </div>
        ...
        <div class="glass-panel p-5 border-l-4 border-l-secondary-blue relative overflow-hidden group hover:translate-y-[-2px] transition-all duration-300">
          ...
          <div class="space-y-1">
            ...
            <div class="flex items-baseline gap-2">
              ...
              <span class="text-xs text-secondary-blue">{{stats().linkedPercentage}}%</span>
            </div>
          </div>
          <div class="absolute -right-4 -bottom-4 opacity-5 group-hover:opacity-10 transition-opacity">
        ...
        <div class="glass-panel p-5 border-l-4 border-l-danger relative overflow-hidden group hover:translate-y-[-2px] transition-all duration-300">
          ...
          <div class="absolute -right-4 -bottom-4 opacity-5 group-hover:opacity-10 transition-opacity">
            <mat-icon class="text-7xl">error_outline</mat-icon>
          </div>
        </div>
        ...
        <div class="glass-panel p-5 border-l-4 border-l-secondary-green relative overflow-hidden group hover:translate-y-[-2px] transition-all duration-300">
          ...
          <div class="space-y-1">
            <h3 class="text-text-tertiary text-xs font-semibold uppercase tracking-wider">Engagement</h3>
            <div class="flex items-baseline gap-2">
              <span class="text-2xl font-bold text-white">High</span>
              <span class="text-xs text-secondary-green">+12% this month</span>
            </div>
          </div>
          <div class="absolute -right-4 -bottom-4 opacity-5 group-hover:opacity-10 transition-opacity">
      ...
      <div class="glass-panel overflow-hidden">
        <div class="p-6 border-b border-border-default flex items-center justify-between">
          <h2 class="text-lg font-semibold text-white">Student GitHub Directory</h2>
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-white/5">
                ...
                <th class="p-4 text-xs font-bold text-text-secondary uppercase tracking-wider">GitHub ID</th>
            ...
            <tbody class="divide-y divide-border-default">
              @for (student of filteredStudents(); track student.id) {
                <tr class="hover:bg-white/5 transition-colors group">
                  ...
                  <td class="p-4">
                    @if (student.github) {
                      <div class="flex items-center gap-2 text-accent-primary">
                        ...
                        <span class="text-sm font-mono">{{student.github}}</span>
                      </div>
                    } @else {
                  ...
                  <td class="p-4">
                    @if (student.github) {
                      <a [href]="'https://github.com/' + student.github" target="_blank" class="w-8 h-8 rounded-lg bg-bg-elevated border border-border-default flex items-center justify-center text-text-secondary hover:text-accent-primary hover:border-accent-primary/50 transition-all">
...
export class GithubComponent {
  ...
  stats = computed(() => {
    ...
    const linked = students.filter(s => s.github && s.github !== 'None').length;
